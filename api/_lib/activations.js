...
const MAP = {
  ...
  /* ------------------------------------------------------------------
     ...
     [เกิดขึ้นจริงแล้ว 24 ก.ย. 2026] เทอมินอล 21 โคราช (24 ก.ย.–5 ต.ค.) กับ
     Gateway บางซื่อ (19–27 ก.ย., งานแทรกเข้ามาทีหลัง) ซ้อนกันช่วง 24–27 ก.ย.
     ของจริง 2 ชุดที่พิมพ์ไปแล้วใช้ QR ใบเดียวกันทั้งคู่ (token 'xstand-a-website')
     แก้ตอนนี้ไม่ทันเพราะพิมพ์ QR ไปแล้ว — ช่วง 24–27 ก.ย. รายงานจะขึ้นเป็น
  ...
  /* ------------------------------------------------------------------
     หน่วย B — เตรียมไว้ล่วงหน้าสำหรับตอนที่ต้องวาง X-stand พร้อมกับหน่วย A
     (ดูหมายเหตุยาวด้านบนในหน่วย A ว่าทำไมต้องแยกต่อหน่วยแทนที่จะแยกต่อห้าง)

     confirmed:false เพราะยังไม่มี QR จริงให้สแกน — ต้อง:
       1. เจน QR ใหม่จาก URL https://vinko.quest/r/xstand-b-website
       2. พิมพ์สติกเกอร์แปะทับ QR เดิมบน X-stand ชุดที่ 2 (หรือพิมพ์ใหม่)
       3. สแกนของจริงแล้วตรวจว่าไปหน้า vinko.quest ถูกโดเมน
       4. ค่อยตั้ง confirmed:true ที่รอบนั้น (เหมือนที่ทำกับ poster ตอน 17 ก.ย. 2026)
     ก่อนถึงขั้นตอนนั้น token นี้จะตอบ 404 เสมอ ไม่มีผลกับ QR หน่วย A ที่ใช้อยู่
     ------------------------------------------------------------------ */
  'xstand-b-website': [
    {
      activation_id: 'QR-XS-B-WEB-01__PENDING__r01',
      active: true,
      ...
      utm: {
        ...
        utm_medium:   'xstand',
        utm_campaign: 'evergreen',
        utm_content:  'qr_xs_b_web_01'
      },
      note: 'หน่วย B — ยังไม่มี QR จริง เตรียม token ไว้ล่วงหน้าเผื่อวางพร้อมหน่วย A'
    }
  ],
  ...
  /* ------------------------------------------------------------------
     ทดลองใหม่ (24 ก.ย. 2026) — หน่วย B เท่านั้น เป็นการทดลอง

     ตอนยกเลิก xstand-a-line ไปตอน 9 ก.ย. 2026 สาเหตุจริงคือทีมงานยัง
     งงกับระบบ /r/ ไม่ใช่เพราะพังทางเทคนิค ดีไซน์เดิม (dest: '/free-sample')
     ก็ถูกต้องอยู่แล้ว: /free-sample เป็นหน้าเว็บของเราเอง ติด GA4 จริง คนละ
     กรณีกับที่คอมเมนต์เก่าเปรียบเทียบไว้ (redirect ตรงไป LINE ไม่ผ่านหน้าเว็บ
     เราเลย ซึ่งกรณีนั้นถึงจะไม่ได้ attribution เพิ่ม) — /free-sample มี Step 1
     "เพิ่มเพื่อน LINE" + Step 2 "โหลดตัวอย่างฟรี" อยู่แล้ว ใช้ได้เลยไม่ต้องสร้างใหม่

     ทดลองเฉพาะหน่วย B (ยังไม่ได้แปะที่ไหนจริง พอดีกับตอนพิมพ์สติกเกอร์เว็บไซต์
     หน่วย B รอบนี้อยู่แล้ว ไม่ต้องเดินไปแปะเพิ่มอีกรอบ) หน่วย A/C ยังใช้ QR ตรง
     ของ LINE OA Manager เหมือนเดิม เทียบผลกันได้ว่าต่างกันแค่ไหน

     confirmed:false จนกว่าจะพิมพ์ QR จริง แปะทับ QR LINE เดิมบนหน่วย B
     แล้วสแกนตรวจว่าไปหน้า /free-sample จริงก่อน
     ------------------------------------------------------------------ */
  'xstand-b-line': [
    {
