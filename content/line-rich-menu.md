...
| 4 | ล่าง-ซ้าย | สั่งซื้อหนังสือ | Link | `https://vinko.quest/books?openExternalBrowser=1` |
...
**เปลี่ยนแล้ว (2026-09-12): ช่อง 4 เดิมคือ `/#order` (หน้าขายชุด) → สลับเป็น `/books`**
`/books` คือหน้าเลือกซื้อ VINKO STORIES แยกเล่ม (เล่มละ 59 บาท) — เลือกได้ทีละเล่ม
หรือหลายเล่ม ถ้าติ๊กครบทั้ง 5 เล่ม หน้านี้จะเตือนเองว่าไปซื้อครบชุดถูกกว่า พร้อมปุ่มลิงก์
ไปหน้าชุด `/checkout?pkg=stories` ให้เลย — ไม่ต้องมีช่องที่ 7 แยกสำหรับซื้อชุด
ต้องมี `?openExternalBrowser=1` เหมือนช่องอื่นที่ลิงก์ไปเว็บ เพราะเปิดจากเมนูรวมของ LINE เสมอ
